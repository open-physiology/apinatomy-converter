...
import os
import gspread
from rdflib import Graph, RDF, URIRef, store, Namespace, RDFS
import rdflib_neo4j
...
query_cql_partial_order = '''
    SELECT ?entry WHERE {
        ?uri ilxtr:neuronPartialOrder ?entry .
    }
'''

neurons = graph.query(query)

for neuron in neurons:
    uri = neuron[0]
    baseID = uri.replace("http://uri.interlex.org/tgbugs/uris/readable/sparc-nlp/", "")
    # Define the class URI
    #class_uri = URIRef('mmset1:1')

    # Query for the label
    baseName = str(graph.value(subject=uri, predicate=RDFS.label))[:50]

    res = graph.query(query_cql_partial_order, initBindings={'uri': uri})
    entries = [record['entry'] for record in res]
